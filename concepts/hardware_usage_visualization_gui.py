...
class HardwareUsageVisualizationApp(customtkinter.CTk):
    def __init__(self):
        super().__init__()

        # Configuring Window Attributes
        self.title('Hardware Usage Visualization')
        self.geometry("640x480")

        # Main Menu Buttons
        self.show_visualizations_button = customtkinter.CTkButton(self, text='Show Hardware Visualizations', command=self.show_visualizations)
        self.show_visualizations_button.grid(row=0, column=0, padx=10, pady=5, sticky='w')

    def show_visualizations(self):
        visualizations_window = VisualizationsWindow(self)

    
class VisualizationsWindow(customtkinter.CTkToplevel):
    def __init__(self, master):
        ...
        # self.gpu_figure, self.gpu_ax = plt.subplots(figsize=(5, 3), tight_layout=True)
        # self.gpu_line, = self.gpu_ax.plot([], [], label='GPU Usage')
        # self.gpu_ax.set_xlabel('Time (s)')
        # self.gpu_ax.set_ylabel('GPU Usage (%)')
        # self.gpu_ax.legend(loc='upper left')

        # Setting limit of y-axis from 0 to 100
        # self.gpu_ax.set_ylim(0, 100)

        # self.gpu_canvas = FigureCanvasTkAgg(self.gpu_figure, self)
        # self.gpu_canvas.get_tk_widget().grid(row=1, column=0, columnspan=2, padx=10, pady=5, sticky='w')
    ...
    # def update_gpu_info(self):
        # try:
            ...
            # Updating the Lineplot
            # x_data = self.gpu_line.get_xdata()
            # y_data = self.gpu_line.get_ydata()

            # x_data = list(x_data) + [time.time() - self.start_time]
            # y_data = list(y_data) + [gpu_percent]

            # self.gpu_line.set_xdata(x_data)
            # self.gpu_line.set_ydata(y_data)

            # Limitting data points to the last 10 seconds
            # self.gpu_ax.set_xlim(max(0, time.time() - self.start_time - 10, time.time() - self.start_time()))
...
if __name__ == '__main__':
    app = HardwareUsageVisualizationApp()
    app.mainloop()
